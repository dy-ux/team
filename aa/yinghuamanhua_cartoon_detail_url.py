...
class BaseCartoon(BaseSpider):
    ...
    def parse_search_qin_quan(self, response, **kwargs) -> list:
        try:
            ...
            # *
            result_list = []
            qin_quaq_list_data = response_data.xpath('//*[@id="detail-list-select-1"]/li')
            for q_l in qin_quaq_list_data:
                dic_info = {}
                qin_quan_url = "http://www.wujicg.com" + ''.join(q_l.xpath('./a/@href'))  # 侵权链接
...
if __name__ == '__main__':
    info = detail_url(url="http://www.wujicg.com/rexue/32274/")
    print(len(info))
    print(info)
